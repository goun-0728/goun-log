...
export default function SectionEditor({ sec, idx, onUpdate }) {
  ...
  useEffect(() => {
    const calc = () => {
      if (wrapRef.current) setScale(Math.min(1, wrapRef.current.offsetWidth / 860))
    }
    ...
    <div style={{ marginBottom:20,borderRadius:12,overflow:'hidden',border:`2px solid ${editing?'#3b82f6':C.bd}`,transition:'border-color .2s' }}>
          ...
          <div style={{ display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:5,marginBottom:14 }}>
            {TPL_LABELS.map(({k,l})=>{const on=dr.template===k;return(
              <button key={k} onClick={()=>{const nx={...dr,template:k};setDr(nx);onUpdate(idx,nx)}}
                style={{ padding:'7px 6px',fontSize:11,borderRadius:8,border:`1.5px solid ${on?'#3b82f6':C.bd}`,background:on?'#EFF6FF':C.sur,color:on?'#1d4ed8':C.mu,cursor:'pointer',fontWeight:on?700:400 }}>{l}</button>
            );})}
          </div>
      ...
      {editing && (
        <div style={{ padding:'7px 14px',background:'#EFF6FF',borderTop:'1px solid #BFDBFE',fontSize:11,color:'#1d4ed8' }}>
          ✏️ 텍스트 클릭 수정 · 이미지 클릭 업로드 · <strong>+ 블록 추가</strong>로 이미지/텍스트를 원하는 위치에 삽입 · 저장 후 PNG 다운로드
        </div>
      )}
    </div>
  )
