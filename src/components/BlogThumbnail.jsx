...
function getTextY(textPosition) {
  ...
  if (textPosition === 'center') return 190
  return 370  // bottom
}
...
function ThumbBg({ t }) {
  ...
  const IS = { width: '100%', height: '100%', objectFit: 'cover', display: 'block' }

  if (layout === 'gradient') return (
    <>
      <div style={{ position: 'absolute', inset: 0, background: bgColor }} />
      {image && <img src={image} alt="" style={{ position: 'absolute', inset: 0, ...IS }} />}
...
function ThumbText({ t }) {
  ...
  return (
    <div style={{ position: 'absolute', left: PAD, top: ty, maxWidth: maxW, zIndex: 5 }}>
      {t.accentLine && (
...
export default function BlogThumbnail({ blogTitle }) {
  ...
  const dlPNG = async () => {
    ...
    setDl(true)
    // scale transform 제거 후 캡처 (html2canvas 위치 오류 방지)
    ...
    <div style={{ marginTop: 24, marginBottom: 4, borderRadius: 12, overflow: 'hidden', border: `2px solid ${editing ? '#1A3FA3' : C.bd}`, transition: 'border-color .2s' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '8px 14px', background: editing ? '#EBF1FF' : C.alt, borderBottom: `1px solid ${editing ? '#BFDBFE' : C.bd}`, flexWrap: 'wrap', gap: 6 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <span style={{ fontSize: 10, color: C.mu }}>1080 × 600px</span>
        </div>
      ...
      <div style={{ display: 'flex', alignItems: 'stretch' }}>
        ...
        <div ref={wrapRef} style={{ flex: 1, minWidth: 0, position: 'relative', background: '#e0ddd8', overflow: 'hidden' }}>
          <div style={{ paddingTop: `${(THUMB_H / THUMB_W) * 100}%`, position: 'relative' }}>
