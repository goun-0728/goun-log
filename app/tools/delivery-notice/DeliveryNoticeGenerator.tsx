...
const HOLIDAYS: Record<string, string> = {
  ...
  '2027-10-03':'개천절','2027-10-04':'추석연휴','2027-10-05':'추석','2027-10-06':'추석연휴',
  '2027-10-09':'한글날','2027-12-25':'크리스마스',
  // 2028
  '2028-01-01':'새해','2028-02-05':'설연휴','2028-02-06':'설날','2028-02-07':'설연휴',
  '2028-03-01':'삼일절','2028-05-05':'어린이날','2028-06-06':'현충일','2028-08-15':'광복절',
  '2028-09-12':'추석연휴','2028-09-13':'추석','2028-09-14':'추석연휴',
  ...
  '2030-09-20':'추석연휴','2030-09-21':'추석','2030-09-22':'추석연휴',
  '2030-10-03':'개천절','2030-10-09':'한글날','2030-12-25':'크리스마스',
  // 2031
  '2031-01-01':'새해','2031-02-01':'설연휴','2031-02-02':'설날','2031-02-03':'설연휴',
  '2031-03-01':'삼일절','2031-05-05':'어린이날','2031-06-06':'현충일','2031-08-15':'광복절',
  '2031-09-09':'추석연휴','2031-09-10':'추석','2031-09-11':'추석연휴',
  '2031-10-03':'개천절','2031-10-09':'한글날','2031-12-25':'크리스마스',
  // 2032
  '2032-01-01':'새해','2032-01-22':'설연휴','2032-01-23':'설날','2032-01-24':'설연휴',
  '2032-03-01':'삼일절','2032-05-05':'어린이날','2032-06-06':'현충일','2032-08-15':'광복절',
  '2032-09-27':'추석연휴','2032-09-28':'추석','2032-09-29':'추석연휴',
  '2032-10-03':'개천절','2032-10-09':'한글날','2032-12-25':'크리스마스',
  // 2033
  '2033-01-01':'새해','2033-01-10':'설연휴','2033-01-11':'설날','2033-01-12':'설연휴',
  '2033-03-01':'삼일절','2033-05-05':'어린이날','2033-06-06':'현충일','2033-08-15':'광복절',
  '2033-09-15':'추석연휴','2033-09-16':'추석','2033-09-17':'추석연휴',
  '2033-10-03':'개천절','2033-10-09':'한글날','2033-12-25':'크리스마스',
  // 2034
...
export default function DeliveryNoticeGenerator() {
  ...
  const draw = useCallback(() => {
    ...
    // 상단 바
    ctx.fillStyle = c.accent; ctx.fillRect(0,0,W,6); curY = 6
    ...
    ctx.fillText(c.title, W/2, curY+40)
    curY += 40 + PAD
    ...
    if (greetLines.length > 0) {
      ...
      ctx.textAlign = 'center'
      greetLines.forEach(line => { ctx.fillText(line, W/2, curY+24); curY += 36 })
      curY += PAD * 0.5
      ctx.strokeStyle = c.border; ctx.lineWidth = 1
      ctx.beginPath(); ctx.moveTo(tx, curY); ctx.lineTo(W-tx, curY); ctx.stroke()
      curY += PAD * 0.8
    ...
    if (hasTime) {
      ...
      ctx.fillText(timeStr, tx+44+dw+12, curY+72)
    }
    curY += BOXH + 14
    ...
    // 달력
    const calYear2 = lastDate ? lastDate.getFullYear() : calYear
    const calMonth2 = lastDate ? lastDate.getMonth() : calMonth
    calRectRef.current = {x:tx, y:curY, w:tw, h:calH, year:calYear2, month:calMonth2}
    ...
    ctx.font = `400 18px ${font}`
    ctx.textAlign = 'center'
    ctx.fillText('이용해 주셔서 감사합니다', W/2, curY+22)
    curY += 22 + PAD
  ...
  function drawCal(
    ...
    ctx.textAlign = 'center'
    ctx.fillText(`${year}년 ${month+1}월`, cx+cw/2, cy+pad+20)

    ctx.fillStyle = c.textSub
    ctx.font = `500 18px ${font}`
    ctx.fillText('‹', cx+pad+10, cy+pad+20)
    ctx.fillText('›', cx+cw-pad-10, cy+pad+20)
    ...
    allDays.forEach(({d:thisD,cur},idx) => {
      ...
      if (isLast) {
        rrect(ctx, dx+2, dy+2, cellW-4, cellH-4, 6)
      ...
      ctx.fillStyle = nc
      ctx.font = `${isLast||isResume?'700':'400'} 15px ${font}`
      ctx.textAlign = 'center'
      ...
      if (isLast||isResume) {
        ...
        ctx.fillText(isLast?'마감':'발송', dx+cellW/2, dy+cellH*0.87)
      }
      ctx.restore()
    })
  }
  ...
  function handleCanvasClick(e: React.MouseEvent<HTMLCanvasElement>) {
    const cv = canvasRef.current
    if (!cv || !calRectRef.current) return
    const rect = cv.getBoundingClientRect()
    const scale = scaleRef.current
    const x = (e.clientX - rect.left) / scale
    const y = (e.clientY - rect.top) / scale

    const cal = calRectRef.current
    const pad = 20
    const cellW = Math.floor((cal.w - pad*2) / 7)
    const headerH = 44
    const dowH = 28
    const cellH = Math.floor((cal.h - pad*2 - headerH - dowH) / 6)
    const gridStartY = cal.y + pad + headerH + dowH

    // 화살표 클릭
    if (y >= cal.y + pad - 10 && y <= cal.y + pad + headerH - 10) {
      if (x >= cal.x + pad && x <= cal.x + pad + 32) {
        if (calMonth === 0) { setCalMonth(11); setCalYear(yr => yr-1) }
        else setCalMonth(m => m-1)
        return
      }
      if (x >= cal.x + cal.w - pad - 32 && x <= cal.x + cal.w - pad) {
        if (calMonth === 11) { setCalMonth(0); setCalYear(yr => yr+1) }
        else setCalMonth(m => m+1)
        return
      }
    }
    ...
    const startDow = first.getDay()
    const lastD = new Date(cal.year, cal.month+1, 0).getDate()
  ...
  return (
    <>
      <style>{`
        ...
        .panel::-webkit-scrollbar-thumb { background: #e0e0e0; border-radius: 4px; }
        .card { background:white; border:1px solid #e8e8e8; border-radius:14px; padding:14px; }
        ...
        .mbtn { padding:7px 4px; font-size:12px; border:1.5px solid #e8e8e8; border-radius:8px; background:white; color:#555; cursor:pointer; text-align:center; font-family:inherit; }
        .mbtn.active { border-color:#534AB7; color:#534AB7; background:#EEEDFE; font-weight:600; }
        .upload-zone { border:1.5px dashed #d0d0d0; border-radius:10px; padding:12px; text-align:center; cursor:pointer; font-size:12px; color:#aaa; }
        .upload-zone:hover { background:#f8f8f8; }
        .chips { display:flex; flex-direction:column; gap:5px; margin-top:8px; }
        .chip { display:flex; align-items:center; gap:6px; font-size:11px; padding:5px 8px; border-radius:8px; background:#f8f8f8; }
        .dot { width:8px; height:8px; border-radius:50%; flex-shrink:0; }
        .chip-lbl { color:#999; }
        .chip-val { color:#222; font-weight:600; margin-left:auto; font-size:11px; }
