...
export const sampleArticles: Article[] = [
  ...
  },
  {
    ...
    content:
      "## AI가 잘하는 일\n\nAI는 초안 작성, 문구 변형, 구조 정리처럼 반복적인 작업에서 큰 도움을 줍니다. 특히 상품 특징을 여러 관점으로 풀어내는 데 강점이 있습니다.\n\n하지만 AI가 만든 결과를 그대로 쓰기보다 판매자가 알고 있는 고객 맥락을 더해야 합니다.\n\n## 사람이 결정해야 하는 것\n\n- 어떤 고객을 우선 설득할지\n- 어떤 장점을 가장 앞에 둘지\n- 어떤 불안을 먼저 해소할지\n- 브랜드의 말투를 어떻게 유지할지\n\nAI는 제작 속도를 높여주는 도구입니다. 방향을 정하는 일은 여전히 사람의 몫입니다.",
    image_url: null,
    status: "published",
...
async function ensureUniqueSlug(slug: string, excludeId?: string) {
  ...
  while (true) {
    ...
    if (error || !data?.length) return candidate;
...
export async function createArticle(input: ArticleInput) {
  ...
  const { error } = await supabase.from("articles").insert({ ...input, slug, published_at: publishedAt });
  if (error) throw error;
}
...
export async function updateArticle(id: string, input: ArticleInput) {
  ...
  const { error } = await supabase
    .from("articles")
    .update({ ...input, slug, published_at: publishedAt, updated_at: new Date().toISOString() })
    .eq("id", id);
